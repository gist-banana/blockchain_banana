  ...
  # 'amount':response.json()[0]['amount']})
    ...
    def new_block(self, proof, previous_hash):
        ...
        if (len(self.chain) != 0):
            ...
            previous_hash = sha256(f'{last_proof}{proof}{previous_hash}'.encode()).hexdigest()
...
#        return guess_hash[:6] == "000000"
    ...
    def test_sendtransact(self, this_input):
...
@app.route('/test_sendtransact', methods=['GET'])
def test_sendtransact():
    this_input = 1
    blockchain.test_sendtransact(this_input)
    #return jsonify(response), 201
    return 0
