    ...
    def bring_current_transactions(self):
        ...
        while True:
            for node in neighbours:
                ...
                print('\n\n test')
                print(len(response.json()['current_transactions']))
                print('\n counter')
                print(counter)
                if counter == len(response.json()['current_transactions']):
                    print('it is a match')
                    print('\n\n')
                    print(response.json()['current_transactions'])
                    self.current_transactions.extend(response.json()['current_transactions'])
                    counter += 1
                    if response.json()['current_transactions'][counter-2]['recipient'] == self.address:
                        self.bitcoin_left += response.json()['current_transactions'][counter-2]['amount']
                        print('\nmy bitcoin : ')
                        print(self.bitcoin_left)
                        continue
                    else:
                        continue
                    continue
...
@app.route('/mass_bring_current_transactions', methods=['GET'])
def mass_bring_current_transactions():
    while 1:
        blockchain.bring_current_transactions()
...
@app.route('/show_me_the_money', methods=['GET'])
def show_me_the_money():
    print('\n\n')
    print('money left : ')
    print(blockchain.bitcoin_left)
        #wallet = BitcoinWallet()
    print('private key : ')
    print(blockchain.private_key)
    print('\n')
